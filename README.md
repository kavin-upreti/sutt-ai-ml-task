...
The data fetched is received from two APIs:
...
HN threads are trees. Many comments have replies to them as well. The approach of splitting by token count (a token is roughly 3/4th of a word) destroys conversation's context. A reply loses meaning when separated from what it was replying to. Hence, this approach was discarded.<br>Instead, we implemented:
...
**Preserving context**: Each comment has a `parent` field from the API itself, which is the id of the comment or the story it directly replied to, and a `root_id` field manually introduced, which is the id of the parent comment it belongs to. After structuring the comment according to our needs, the comments are sorted with relevance in the order `(root_id, parent, position, upvotes)` so that the replies to a comment are near the other replies to the same comment. Root ID (which is the ID of the comment at position 0) being first implies that all of the replies of that comment will have the same ID and will be grouped together, then all of the same parent ID (the one they are directly replying to) are taken into consideration rather than being scattered. The motive is not to sort them in an ascending or descending order - it is to sort them in groups of the same IDs. This builds a reasonable thread order and is easy for the AI to interpret.
...
- Everytime the user chats, the LLM receives the digest (upto 1500 characters), the conversation summary, the exact recent messages, and the top 5 comments for reference
...
B. **Contradictory opinions in the data**: The prompt explicitly tells the LLM to present both sides from a neutral perspective when the discussion has conflicting views instead of settling on one of them.<br>
